"""Availability computation (HTTP-agnostic)."""
...
async def compute_availability(
    ...
    *,
    ...
    if service is None:
        raise AvailabilityServiceError(404, "Service not found")
    ...
    if (
        ...
        raise AvailabilityServiceError(
            422, "Service duration must be aligned to slot interval"
        )

    if employee_id is not None:
        employee = await session.get(Employee, employee_id)
        if employee is None:
            raise AvailabilityServiceError(404, "Employee not found")
        ...
        if relation is None:
            raise AvailabilityServiceError(
                400, "Employee does not perform this service"
            )
